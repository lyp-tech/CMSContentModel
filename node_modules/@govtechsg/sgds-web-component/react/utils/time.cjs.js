...
const sanitizedNextMonth = (d) => {
    ...
    const numberOfDaysNextMonth = dateFns.getDaysInMonth(nextMonthDate);
    if (dateFns.isLastDayOfMonth(d) && numberOfDaysNextMonth < date) {
        return dateFns.lastDayOfMonth(nextMonthDate);
    }
    else {
...
const sanitizedPreviousMonth = (d) => {
    ...
    const numberOfDaysPrevMonth = dateFns.getDaysInMonth(prevMonthDate);
    if (dateFns.isLastDayOfMonth(d) && numberOfDaysPrevMonth < date) {
        return dateFns.lastDayOfMonth(prevMonthDate);
    }
    else {
        return new Date(year, month - 1, date);
    }
...
exports.createYearViewArray = createYearViewArray;
exports.sanitizedNextMonth = sanitizedNextMonth;
exports.sanitizedPreviousMonth = sanitizedPreviousMonth;
exports.setTimeToNoon = setTimeToNoon;
//# sourceMappingURL=time.cjs.js.map
